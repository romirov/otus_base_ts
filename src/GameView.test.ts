...
describe("GameView", () => {
  ...
  describe("functional interface", () => {
    ...
    it("renders correct game state on .updateGameState", () => {
      ...
      let buttonState = el.querySelector(".run-button.run-button--stopped")
      if(buttonState != null){
      expect(
      ...
      buttonState = el.querySelector(".run-button.run-button--runned")
      if(buttonState !== null)
        expect(buttonState.innerHTML).toBe(
          "Stop"
        );
      expect(
        Number(
